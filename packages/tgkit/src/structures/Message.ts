...
export default class Message extends Base {
	...
	constructor(client: Client, data: any) {
		...
		this.senderBusinessBot =
			data.sender_business_bot && new User(client, data.sender_business_bot);
		...
		this.externalReply = data.external_reply && {
			...
			animation:
				data.external_reply.animation &&
				new Animation(client, data.external_reply.animation),
			audio: data.external_reply.audio && new Audio(client, data.external_reply.audio),
			document:
				data.external_reply.document && new Document(client, data.external_reply.document),
			photo:
				data.external_reply.photo &&
				data.external_reply.photo.map((photoData: any) => new PhotoSize(client, photoData)),
			sticker:
				data.external_reply.sticker && new Sticker(client, data.external_reply.sticker),
			...
			videoNote:
				data.external_reply.video_note &&
				new VideoNote(client, data.external_reply.video_note),
			...
			contact:
				data.external_reply.contact && new Contact(client, data.external_reply.contact),
			...
			giveaway:
				data.external_reply.giveaway && new Giveaway(client, data.external_reply.giveaway),
			giveawayWinners: data.external_reply.giveaway_winners && {
				...
				winnersSelectionDate: unixToDate(
					data.external_reply.giveaway_winners.winners_selection_date,
				),
				winnerCount: data.external_reply.giveaway_winners.winner_count,
				winners: data.external_reply.giveaway_winners.winners.map(
					(winnerData: any) => new User(client, winnerData),
				),
				additionalChatCount: data.external_reply.giveaway_winners.additional_chat_count,
				premiumSubscriptionMonthCount:
					data.external_reply.giveaway_winners.premium_subscription_month_count,
		...
		this.entities =
			data.entities &&
			data.entities.map((entityData: any) => new MessageEntity(this.client, entityData));
		...
		this.photo =
			data.photo && data.photo.map((photoData: any) => new PhotoSize(this.client, photoData));
		...
		this.newChatPhoto =
			data.new_chat_photo &&
			data.new_chat_photo.map((photoData: any) => new PhotoSize(this.client, photoData));
		...
		this.proximityAlertTriggered = data.proximity_alert_triggered && {
			traveler:
				...
				new User(this.client, data.proximity_alert_triggered.traveler),
			watcher:
				data.proximity_alert_triggered.watcher &&
				new User(this.client, data.proximity_alert_triggered.watcher),
		...
		this.giveawayWinners = data.giveaway_winners && {
			...
			winners: data.giveaway_winners.winners.map(
				(winnerData: any) => new User(this.client, winnerData),
			),
		...
		this.giveawayCompleted = data.giveaway_completed && {
			...
			giveawayMessage:
				data.giveaway_completed.giveaway_message &&
				new Message(this.client, data.giveaway_completed.giveaway_message),
		};
		...
		this.videoChatParticipantsInvited = data.video_chat_participants_invited && {
			users: data.video_chat_participants_invited.users.map(
				(userData: any) => new User(this.client, userData),
			),
		};
